    ...
    void switch_option(Button bttn)
    {
        ...
        for (int i = 0; i < bttns.Length; i++)
        {
            ...
            if (bttns[i] != bttn)
            {
                ...
                LeanTween.moveY(sub_menus[i].GetComponent<RectTransform>(), -350, 1f).setEase(LeanTweenType.easeInQuad); ;
            ...
            else
            {
                ...
                LeanTween.moveY(sub_menus[i].GetComponent<RectTransform>(), -180, 1.5f).setEase(LeanTweenType.easeOutQuad); ;
